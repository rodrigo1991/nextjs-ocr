...
export async function splitIntoLineImages(
  ...
  const src = await cvImread(image);
  ...
  for (let i = 0; i < contours.size(); i++) {
    ...
    const { points, sside } = await getMiniBoxes(cnt);
    ...
    const resultObj = await getMiniBoxes(boxMap);
    ...
    const c = await getRotateCropImage(srcData, box);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function getMiniBoxes(contour: any) {
  const cv = await getOpenCv();
...
async function getRotateCropImage(
  imageRaw: ImageRawType,
  points: BoxType
): Promise<ImageRawType> {
  const cv = await getOpenCv();
  const img_crop_width = int(
    Math.max(linalgNorm(points[0], points[1]), linalgNorm(points[2], points[3]))
  );
  const img_crop_height = int(
    Math.max(linalgNorm(points[0], points[3]), linalgNorm(points[1], points[2]))
  );
  const pts_std = [
    [0, 0],
    [img_crop_width, 0],
  ...
  const src = await cvImread(imageRaw);
...
function polygonPolygonLength(polygon: pointsType) {
  ...
  let [xb, yb] = polygon[n - 1]; // last point

  for (let i = 0; i < n; i++) {
    const [xNext, yNext] = polygon[i];
    const dx = xb - xNext;
    const dy = yb - yNext;
...
async function cvImread(image: ImageRawType) {
  const cv = await getOpenCv();
  return cv.matFromImageData(image);
}

function cvImshow(mat: Mat): ImageRawType {
